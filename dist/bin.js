(function (global, factory) {
  ...
  var class2type = {}; //eslint-disable-next-line
  ...
  function matches(element, selector) {
    if (!selector || !element || element.nodeType !== 1) return false; // matches 方法用于检测元素( element )是否匹配特定的选择器( selector )
  ...
  function likeArray(obj) {
    var length = !!obj && 'length' in obj && obj.length; // length === 0 其实就是将其看作为空数组
    // 最后一种情况必须要满足三个条件：
    // 1.length 必须为数字
    // 2.length 必须大于 0 ，表示有元素存在于类数组中
    // 3.key length - 1 必须存在于 obj 中。我们都知道，数组最后的 index 值为 length -1 ，这里也是检查最后一个 key 是否存在。

    return !isFunction(obj) && !isWindow(obj) && (isArray(obj) || length === 0 || typeof length == 'number' && length > 0 && length - 1 in obj);
  } //eslint-disable-next-line


  function fragment(html, name, properties) {
    var dom, container; // A special case optimization for a single tag
  ...
  function qsa(element, selector) {
    ...
    return element.getElementById && isSimple && maybeID ? //eslint-disable-next-line
    (found = element.getElementById(nameOnly)) ? [found] : [] : element.nodeType !== 1 && element.nodeType !== 9 && element.nodeType !== 11 ? // 1 元素节点, 9 Document节点, 11 DocumentFragment节点
  ...
  Bin.prototype = {
    ...
    attr: function attr(name, value) {
      var result;
      return typeof name == 'string' && !(1 in arguments) ? 0 in this && this[0].nodeType == 1 && (result = this[0].getAttribute(name)) != null ? result : undefined : this.each(function () {
        if (this.nodeType !== 1) {
          return;
        }
    ...
    offset: function offset() {
      ...
      if (docElem !== this[0] && !$.contains(docElem, this[0])) return {
        top: 0,
        left: 0
      };
    ...
    toggleClass: function toggleClass(value) {
      if (!value) return;
      var rspace = /\s+/; // toggle individual class names
  ...
  $.fn = Bin.prototype // Generate the `width` and `height` functions
  ;
  ...
  function () {
    ...
    _proto.getPixel32 = function getPixel32() {}
    /**
     * 获取指定像素区域的颜色值
     * */
    ;

    _proto.getPixels = function getPixels(x, y, width, height) {
    ...
    _proto.saveToFile = function saveToFile() {}
    /**
